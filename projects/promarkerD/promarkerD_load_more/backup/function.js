jQuery(function ($) {
      ...
      success: function (response) {
        if (response) {
          setTimeout(function() {
            ...
            $(".announcements_sec .sec_content .filtered_content").replaceWith(response);
          }, 500);
        }
      },
    });
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response) {
          setTimeout(function() {
            $(".loader_wrap").removeClass("active");
            $(".tabs-section .tab-content .announcements_posts").replaceWith(response);
          }, 500);
        }
      },
    });
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response) {
          setTimeout(function() {
            $(".loader_wrap").removeClass("active");
            $(".tabs-section .tab-content .presentation_posts").replaceWith(response);
          }, 500);
        }
      },
    });
  ...
  // on announcement page media load more post:-
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response) {
          setTimeout(function() {
            $(".loader_wrap").removeClass("active");
            $(".tabs-section .tab-content .tab-pane .media_posts").replaceWith(response);
          }, 500);
        }
      },
    });
  ...
  $('#post_date_filter').on('change', function() {
    ...
    let get_data_attr_value = $("#pills-tab .nav-link.active").attr('data-tab-name');
            ...
            if (response) {
                setTimeout(function() {
                    if (get_data_attr_value == "announcements") {
                        $(".loader_wrap").removeClass("active");
                        $( ".tabs-section .tab-content .tab-pane .announcements_posts" ).replaceWith(response);
                    }else if(get_data_attr_value == "presentations"){
                        $(".loader_wrap").removeClass("active");
                        $( ".tabs-section .tab-content .tab-pane .presentation_posts" ).replaceWith(response);
  ...
  $(".clinical_content_sec .read_more_btn").click(function() {
    ...
    // Toggle the button text
    if ($this.hasClass("active")) {
      $this.removeClass("active").html(`Read More <img src="/wp-content/uploads/2024/08/red-arrow.png" alt="icon">`);
    } else {
      $this.addClass("active").html(`Read Less <img src="/wp-content/uploads/2024/08/red-arrow.png" alt="icon">`);
    }
  });
});
