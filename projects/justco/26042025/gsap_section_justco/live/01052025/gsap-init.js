function afterPageLoad() {
  ...
  // hero section animation text filling on heading start*******
  const masks = document.querySelectorAll(".hero_tbo .hero_heading .mask");
  const firstMask = masks[0];
  const secondMask = masks[1];
  const initWidth = 280;

  firstMask.style.width = `${initWidth}px`;

  // hero section animation function
  function heroTextFilling(window) {
    const scrollYpos = window.scrollY * 5 + initWidth;
    const maxWidth = 700;

    if (firstMask && secondMask) {
      if (scrollYpos <= maxWidth) {
        firstMask.style.width = `${scrollYpos}px`;
        secondMask.style.width = `0px`;
      } else {
        firstMask.style.width = `${maxWidth}px`;
        const secondWidth = scrollYpos - maxWidth;
        secondMask.style.width = `${Math.min(secondWidth, maxWidth)}px`;
      }
    } else {
      console.warn("Masks not found!");
    }
  }
  // hero section animation text filling on heading end********

  // header filter animate on scroll function start*****
  function headerFilterAnimate(window) {
    const scrollYpos = window.scrollY;
    if (scrollYpos > 780) {
      document.querySelector(".main-header-tm").classList.add("stickyFilters");
    } else {
      document
        .querySelector(".main-header-tm")
        .classList.remove("stickyFilters");
    }
  }
  // header filter animate on scroll function end********

  function scrollFun() {
    heroTextFilling(window); //for hero section
    headerFilterAnimate(window); //for header
  }
